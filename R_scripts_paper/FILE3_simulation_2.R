...
ggplot(data = data.frame(dataset, Cluster = as.factor(G0)), aes(x = X1, y = X2, col = Cluster)) +
  geom_point() +
  theme_light() +
  xlab("") +
  ylab("")
...
ggplot(data = data.frame(dataset, Cluster = as.factor(fit_kdiss$cluster)), aes(x = X1, y = X2, col = Cluster)) +
  geom_point() +
  theme_light() +
  xlab("x") +
  ylab("y")
