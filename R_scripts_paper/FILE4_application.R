...
loss <- 0
for(j in 1:k){ loss <- loss + GBClust:::loss_binary(dataset[G_map ==j,])}
loss
...
data_plot           <- rbind(data.frame(data_plot, Type="Maximum a posteriori"),
                            data.frame(data_plot, Type = "Minimum VI"))
colnames(data_plot) <- c("Var1","Var2","Probability","Type")

dummy_data_plot <- rbind(data.frame(x = c(38.5,38 + 29+0.5), y = c(38.5,38 +29 + 0.5), Type="Maximum a posteriori"),
                         data.frame(x = c(44.5,44 + 23+0.5), y = c(44.5,44 +23 + 0.5), Type="Minimum VI"))

p1_co <- ggplot(data_plot, aes(Var1, Var2, fill = Probability))+ geom_tile()+ scale_fill_distiller(palette="Blues",direction=1) + xlab("Slides") + ylab("Slides")  + theme_light()+ facet_grid(.~Type) + geom_vline(data=dummy_data_plot, aes(xintercept=x), linetype="dashed") + geom_hline(data=dummy_data_plot, aes(yintercept=y),linetype="dashed") 
