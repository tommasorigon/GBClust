...
List kbinary_C(const arma::mat& x, arma::vec G, arma::vec freq, int k, bool trace){
  ...
  for(int j = 0; j < k; j++){
    // Identify the elements within the cluster
    idx_cluster = find(G == j + 1); 
    m.row(j) = freq(j) / (freq(j) + 1)* mean(x.rows(idx_cluster)) + 0.5/(freq(j) + 1);
    //Rprintf("n_k: %f \n", m(j,1));
    total_loss = total_loss + loss_binary(x.rows(idx_cluster));
  }
  ...
  while(!convergence){
    ...
    for(int i = 0; i < n; i++) {
      for(int j = 0; j < k; j++){
        losses(j) = -(sum(x.row(i) % log(m.row(j) / (1 - m.row(j))) +  log(1 - m.row(j))));
      }
      // Compute the probability to move into another cluster
      freq(G(i)-1) += -1;
    ...
    for(int i = 0; i < n; i++) {
      ...
      if(!skip){
        ...
        for(int j = 0; j < K; j++){
          ...
          idx_cluster = find(G == j + 1); // This  does not include G since it has been set to NaN
          arma::uvec idx_cluster_i(freq(j)+1); idx_cluster_i.head(freq(j)) = idx_cluster; idx_cluster_i.tail(1) = i;
          lprob(j) = - lambda*(loss_binary(X.rows(idx_cluster_i)) - loss_binary(X.rows(idx_cluster)));
        }
